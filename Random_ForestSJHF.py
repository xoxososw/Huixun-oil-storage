        ...
        for max_depth in list(range(1,20,2)):  # 第二个数原为20
            ...
            #             for min_samples_split in list(range(2,100,10)):
                            # for max_features in list(range(2,50,2)):
                                ...
                                cnt += 1
                                if cnt % 100 == 0:
                                    print(max_depth, max_leaf_nodes, min_samples_leaf,)
                                # print("第 %d 次" % (iter))
                                ...
                                if bst_acc_tst < acc_test:
                                    ...
                                    # 保存训练的数据模型
                                    fw = open('./models/' + str_col + '/' + 'RF-output/RF_model.pkl', 'wb')
                                    pickle.dump(c1, fw)
                                    fw.close()
                                    ...
                                    for index, model in enumerate(Estimators):
                                        ...
                                        with open('./models/'+str_col+'/'+'RF-output/tmp_dot/' + filename, 'w') as f:
                                    ...
                                    test_y.to_csv('./models/'+str_col+'/'+'RF-output/predict-result.csv', index=False,encoding='utf_8_sig')
                                    ...
                                    with open('./models/'+str_col+'/'+'RF-output/score.txt', 'w') as f:
                                        f.write(score)
                                    ...
                                    plt.savefig('./models/'+str_col+'/'+'RF-output/RF-Confusion Matrix.png')
