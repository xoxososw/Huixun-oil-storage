...
def RF(name):#随机森林挑选top50特征
    ...
    res.to_csv("./特征提取/rf/"+name.split('/')[3],index=False,encoding='utf_8_sig')
...
def pca(name):#降维并融合录井特征
    ...
    for i in [15]:
        ...
        data_pred.to_csv('.\特征提取\pca\\'+name.split('/')[3],index=False,encoding='utf_8_sig')
    return data_pred


def por_transfor(df):
    rocklist = ['粉砂岩', '含砾不等粒砂岩', '砾状砂岩', '泥岩', '泥质粉砂岩', '泥质砂岩', '砂质泥岩', '细砂岩',
                '中砂岩', '含砾泥岩', '粉细砂岩', '砂砾岩', '']
    a = df["rock"].values.tolist()
    df['rock'] = [-2 if i not in rocklist else i for i in a]
    ...
    df['rock'] = df['rock'].replace('含砾泥岩', 9)
    df['rock'] = df['rock'].replace('粉细砂岩', 10)
    df['rock'] = df['rock'].replace('砂砾岩', 11)
    df['rock'] = df['rock'].replace('', -1)

    ygyslist = ['褐色', '灰白色', '灰褐色', '灰黄色', '灰绿色', '灰色', '浅灰色', '浅棕红色', '棕红色', '棕黄色',
                '暗黄', '暗黄色', '淡黄', '淡黄色', '黄', '黄色', '亮黄''黄白', "棕黄", '无', '']
    b = df["ygys"].values.tolist()
    df['ygys'] = [-2 if i not in ygyslist else i for i in b]
    ...
    hyjblist = ['荧光', '油斑', '油迹', '油浸', '饱含油', '富含油', '']
    a = df["hyjb"].values.tolist()
    df['hyjb'] = [-2 if i not in hyjblist else i for i in a]
...
if __name__ == '__main__':
    ...
    col = [['cellid', 'depth', "ac", "cnl", "gr", "sp", "ra25"],
           ['cellid', 'depth', "ac", "cnl", "den", "gr", "sp", "ra25"],
           ['cellid', 'depth', "ac", "cnl", "den", "gr", "sp", "rt"]]

    name = ['./数据预处理后的训练集和测试集/小层+测井数据+录井+训练+5RA25-addShui.csv' ,
            './数据预处理后的训练集和测试集/小层+测井数据+录井+训练+6RA25-addShui.csv',
            './数据预处理后的训练集和测试集/小层+测井数据+录井+训练+6RT-addShui.csv']
